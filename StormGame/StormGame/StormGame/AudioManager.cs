...
public class AudioManager : GameComponent
{
    ...
    /// <param name="targetVolume">Target volume, 0.0f to 1.0f</param>
    /// <param name="duration">Length of volume transition</param>
    /// <param name="nextSong">Song to be played when fade is complete</param>
    public void FadeToNewSong(float targetVolume, TimeSpan duration, string nextSong)
    {
        if (duration <= TimeSpan.Zero)
        {
            throw new ArgumentException("Duration must be a positive value");
            ...
            if (_currentSong != null && MediaPlayer.State == MediaState.Playing)
            {
                if (_fadeEffect.Update(gameTime.ElapsedGameTime))
                {
                    ...
                    if (_quedSong != null)
                    {
                        ...
                        CurrentSong = null;
                        _currentSong = null;
                        PlaySong(_quedSong, true);
                        FadeSong(_fadeEffect.SourceVolume, TimeSpan.FromSeconds(1));
                        _quedSong = null;
                    }
                }
